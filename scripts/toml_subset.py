...
This is a vendored copy of Quidra Core's `scripts/toml_subset.py`; everything
below this docstring is kept byte-identical to it. It is vendored rather than
shared because this repository must not fetch Core's metadata over the network
at build time, and there is no submodule or package-dependency mechanism
between the Quidra repositories.

The subset is deliberately tiny and strictly enforced: Core's
`src/toml_subset.cpp` parses the same grammar in the compiler, and keeping both
readers to one small grammar is what makes a `project.toml` written for one
