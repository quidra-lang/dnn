...
Model model = Model(dense = dnn.Linear(features_in = 2, features_out = 1))
...
dnn.Conv2DLayer convolution = dnn.Conv2D(
    channels_in = 1,
    channels_out = 2,
    kernel = 3,
    padding = 1
...
dnn.BatchNormLayer normalization = dnn.BatchNorm(features = 2)
...
dnn.DropoutLayer masking = dnn.Dropout(rate = 0.5, seed = uint64(17))
...
Model model = Model(dense = dnn.Linear(features_in = 1, features_out = 1))
dnn.AdamOptimizer optimizer = dnn.Adam(rate = 0.1)
...
Model model = Model(dense = dnn.Linear(features_in = 2, features_out = 2))
tensor<float32> samples = tensor.ones<float32>([1, 2])
...
dnn.SGDOptimizer optimizer = dnn.SGD(rate = 0.5)
optimizer.step(&model, gradients)
...
dnn.LinearLayer layer = dnn.Linear(features_in = 3, features_out = 4)
...
dnn.LinearLayer repeated = dnn.Linear(features_in = 3, features_out = 4)
dnn.LinearLayer reseeded = dnn.Linear(features_in = 3, features_out = 4, seed = 7)
